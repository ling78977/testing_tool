...
struct Log FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  bool mutate_level(foxglove::LogLevel _level = static_cast<foxglove::LogLevel>(0)) {
    return SetField<uint8_t>(VT_LEVEL, static_cast<uint8_t>(_level), 0);
  }
  /// Log message
  ...
  bool mutate_line(uint32_t _line = 0) {
    return SetField<uint32_t>(VT_LINE, _line, 0);
  }
...
inline Log *GetMutableLog(void *buf) {
  return ::flatbuffers::GetMutableRoot<Log>(buf);
}

inline foxglove::Log *GetMutableSizePrefixedLog(void *buf) {
  return ::flatbuffers::GetMutableSizePrefixedRoot<foxglove::Log>(buf);
}
