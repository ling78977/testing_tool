...
struct RawImage FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  ::flatbuffers::String *mutable_encoding() {
    return GetPointer<::flatbuffers::String *>(VT_ENCODING);
  }
  /// Byte length of a single row
  uint32_t step() const {
    return GetField<uint32_t>(VT_STEP, 0);
  }
  bool mutate_step(uint32_t _step = 0) {
...
inline RawImage *GetMutableRawImage(void *buf) {
  return ::flatbuffers::GetMutableRoot<RawImage>(buf);
}

inline foxglove::RawImage *GetMutableSizePrefixedRawImage(void *buf) {
  return ::flatbuffers::GetMutableSizePrefixedRoot<foxglove::RawImage>(buf);
}
